...
def get_llm_settings():
    ...
    provider = st.sidebar.selectbox("Select LLM Provider", provider_options, key="provider_select", index=default_index)
    ...
    if provider == "OpenAI":
        ...
        model = st.sidebar.text_input("Model", value=default_model,
                                      help="Enter the OpenAI model to use", key="model_input")
        ...
        if not azure_api_key:
            ...
            azure_api_key = st.sidebar.text_input("Enter your Azure OpenAI API Key", type="password",
                                                  key="azure_api_key", value=default_azure_key)
        else:
        ...
        if not azure_endpoint:
            ...
            azure_endpoint = st.sidebar.text_input("Enter your Azure OpenAI Endpoint",
                                                   key="azure_endpoint", value=default_endpoint)
        settings["azure_endpoint"] = azure_endpoint

        default_model = st.session_state.get("model", "gpt-4o-barak")
        model = st.sidebar.text_input("Model", value=default_model,
                                      help="Enter the Azure OpenAI model to use", key="model_input")
        settings["model"] = model
